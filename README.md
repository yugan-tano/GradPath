...
| **界面外观** | 顶栏一键切换暗色 / 亮色模式，支持中 / 英文界面切换（偏好自动保存） |
| **关联链接** | 任意记录（项目 / 论文 / 实验 / 资料 / 政策 / 课程等）之间可「+」号按需添加跨实体、跨场景链接；项目聚合页用链接自动统计关联项 |
...
└─────────────────────────────────────────────┘
        ...
        ▼               ▼               ▼
┌──────────────┐ ┌──────────────┐ ┌──────────────┐
│ scenes/tuimian│ │ scenes/study  │ │ scenes/...   │
│  推免/保研    │ │  学习场景     │ │ research/    │
│ scene.json   │ │ scene.json   │ │ phd 等更多    │
│ hooks.py     │ │ hooks.py     │ │              │
└──────────────┘ └──────────────┘ └──────────────┘
```

- **`scene.json`**：用声明式 JSON 描述场景的实体字段、导航页、幕-阶段树、检查清单、种子数据、默认设置。
- **`hooks.py`**：承载无法用 JSON 表达的强业务逻辑（文件分类、套磁聚合、总览统计、级联副作用）。
- **加新场景 = 新增 `scenes/<id>/` 目录**，不改动引擎一行代码。
- **场景切换**：顶栏场景切换器一键在推免 / 学习 / 科研 / 申博之间切换，各场景数据（SQLite 与资料目录）相互隔离。
- **关联链接**：引擎内置跨实体、跨场景的「链接」能力（`engine/links.py`，数据存于 `data/links.db`），记录可互相引用，聚合页按链接自动统计，删除记录时级联清理链接。
...
data/
├─ links.db                # 跨实体 / 跨场景链接
├─ scenes/<场景id>/app.db  # 各场景实体数据（SQLite）
├─ backups/
├─ avatar.*
...
| 架构 | 引擎 + 场景包（配置 + 钩子） |
| 数据位置 | 本机 `data/scenes/<场景id>/app.db` |
| 文件位置 | 本机各场景资料目录（如 `保研准备/`、`学习资料/`） |
...
- [x] 数据导出：按实体导出 CSV / Markdown，一键生成全量数据报表
- [x] 界面外观：暗色 / 亮色切换 + 中 / 英文界面（场景内容同步翻译）
- [x] 学习 / 科研 / 申博场景扩展：三套场景包 + 运行时场景切换与数据隔离
- [x] 学习场景：绩点核算与选课统计（学分 / 绩点）+ 学期追踪表
- [x] 科研场景：实验参数 / 变量自定义字段（keyvalue 模块）
- [x] 申博场景：学校政策记录对比 + 英语成绩（雅思 / 托福等）进度表
- [x] 关联链接：跨实体 / 跨场景链接 + 项目聚合页自动统计
...
使用中遇到问题或有改进建议，欢迎通过以下任意方式反馈：

- **GitHub Issues**：[提交 Issue](https://github.com/yugan-tano/GradPath/issues)
- **邮箱**：`1830103500@qq.com`

提交反馈时，请尽量附上场景（推免 / 学习 / 科研 / 申博）、复现步骤和相关截图，方便我们快速定位。

## 欢迎加入我们

GradPath 是一个开源项目，欢迎每一位感兴趣的同学参与共建：
...
你可以 Fork 仓库后提交 Pull Request，也可以直接在 Issues 中说明你的想法。

## 欢迎支持我们

如果 GradPath 对你有帮助，欢迎 **Star** 本项目，让更多人看到它。

项目的后续开发与维护需要时间与精力。若未来开发 / 维护资金不足，我们会启动**募集资金**（众筹 / 赞助）计划，用于支持长期维护与功能迭代。届时会在本仓库与 README 中公告，感谢你的关注与支持。

- GitHub: [yugan-tano/GradPath](https://github.com/yugan-tano/GradPath)
- Email: `1830103500@qq.com`
