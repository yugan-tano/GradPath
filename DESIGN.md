...
> 从零重构为「引擎 + 场景包」分层：`engine/` 只承载通用能力，业务逻辑下沉到 `scenes/<id>/`（`scene.json` 配置 + `hooks.py` 钩子）。本文档记录架构约定与存储分工规则，是所有后续实现步骤的硬约束。
...
`scenes/<id>/scene.json` 描述：场景元信息、实体（字段 + 状态机）、幕-阶段树、清单、模板、时间线。无法用 JSON 表达的强业务逻辑（文件分类、聚合统计、级联副作用）写进 `scenes/<id>/hooks.py`，由引擎按约定名调用。加新场景 = 新增目录，不动引擎。
...
见 `engine/dataroot.py`。六步迁移：暂停写入 → 一致性快照 → 创建并验证新位置 → 迁移并校验 → 原子切换 → 保留回滚副本。回滚仅重指锚点，旧目录保留。
